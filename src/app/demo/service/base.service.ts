...
export class FormService {
    ...
    getFiliere(api: string, faculteId: number, diplomeId: number) {
        return this.http
            .get<any>(
                `${environment.apiUrl}/${api}/faculte?faculteId=${faculteId}&diplomeId=${diplomeId}`,
                this.httpOptions
            )
            .pipe(retry(2), catchError(this.handleError));
    }
    ...
    getFiliereBy(api: string, faculteId: number, diplomeId: number) {
        return this.http
            .get<any>(
                `${environment.apiUrl}/${api}?faculteId=${faculteId}&diplomeId=${diplomeId}`,
                this.httpOptions
            )
            .pipe(retry(2), catchError(this.handleError));
    }
    ...
    getDepartements(api: string, regionId: number) {
        return this.http
            .get<any>(
                `${environment.apiUrl}/${api}?regionId=${regionId}`,
                this.httpOptions
            )
            .pipe(retry(2), catchError(this.handleError));
    }
    ...
    getArrondissement(api: string, departementId: number) {
        return this.http
            .get<any>(
                `${environment.apiUrl}/${api}?departId=${departementId}`,
                this.httpOptions
            )
            .pipe(retry(2), catchError(this.handleError));
    }
    ...
    getParameteres() {
        return this.http
            .get(
                `${environment.apiUrl}/` +
                    "administration/institutions/parametres",
                this.httpOptions
            )
            .pipe(retry(2), catchError(this.handleError));
    }
    ...
    updates(api: string, form: any): Observable<any> {
        ...
        return this.http
            .put<any>(
                `${environment.apiUrl}/${api}/${form.id}`,
                JSON.stringify(form),
                this.httpOptions
            )
            .pipe(retry(2), catchError(this.handleError));
    ...
    delete(api: string) {
        ...
        return this.http
            .delete<any>(`${environment.apiUrl}/${api}`, this.httpOptions)
            .pipe(retry(2), catchError(this.handleError));
    }

    getById(api: string, id: number) {
        return this.http
            .get<any>(`${environment.apiUrl}/${api}/${id}`, this.httpOptions)
            .pipe(retry(2), catchError(this.handleError));
    }
    ...
    getByEnrollement(api: string, type: string) {
        return this.http
            .get<any>(`${environment.apiUrl}/${api}/${type}`, this.httpOptions)
            .pipe(retry(2), catchError(this.handleError));
    }
    ...
    getDataReinscription(url: any, data: any) {
        return this.http
            .get<any>(
                `${environment.apiUrl}/${url}?faculteId=${data.faculteId}&typeReinscription=${data.typeReinscription}&matricule=${data.matricule}&nom=${data.nom}&prenom=${data.prenom}&cni=${data.cni}&email=${data.email}&confirmationEmail=${data.confirmationEmail}`,
                this.httpOptions
            )
            .pipe(retry(2), catchError(this.handleError));
    }
    ...
    generateExcel(title, data) {
        if (data.length > 0) {
            data.map((item) => {
                ...
                if (
                    item.hasOwnProperty("inscriptions") &&
                    item.hasOwnProperty("fullName") &&
                    item.hasOwnProperty("nbrePaiement") &&
                    item.hasOwnProperty("soldeTotal")
                ) {
    ...
    printListe(title: string, data: any) {
        ...
        win.document.write(
            "<html><head><title>" +
                title +
                '</title><link rel="stylesheet" type="text/css" href="../../../print.css"></head><body>'
        );
